...
class SimilarityEvaluator:
    ...
    def __init__(self, model_name, val_set_name, base_dir="feat"):
    ...
    # def find_most_similar(image_similarity_model, val_left, candidates_dict, val_candidates_features):
    ...
    #             similarity = image_similarity_model.compute_similarity(anchor_features, candidate_features)
    #             if similarity > highest_similarity:
    #                 highest_similarity = similarity
    #                 most_similar_index = candidate_index
    #         most_similar_indices[anchor_key] = most_similar_index
    #     return most_similar_indices


    def find_top2_similar(self, image_similarity_model, val_left, candidates_dict, val_candidates_features):
        top2_indices = {}
        for anchor_key, anchor_features in val_left.items():
    ...
    def evaluate_accuracy(self, image_similarity_model, val_dict):
        val_left, augmented_val_dict, val_candidates_features = self.load_validation_data(val_dict)
        # most_similar_indices = self.find_most_similar(
        #     image_similarity_model, val_left, augmented_val_dict, val_candidates_features)
